...
<body>
	...
	<div class="row">
		...
		<table class="table table-bordered table-hover">
			...
			<tr><td><a href="${pageContext.request.contextPath}/upload/${nf.noticefileName}">${nf.noticefileName}</a></td></tr>
			</c:if>
			</c:forEach>
		</table>
		<!-- 댓글 테이블 -->
		...
		<table class="table table-sm table-bordered table-hover">
			...
			<tr>
				...
				<td style="width:100px; text-align:center">
					<button class="btn btn-outline-danger btn-sm" type="button" onclick="location.href='${pageContext.request.contextPath}/admin/removeComment/${c.commentId}/${notice.noticeId}'">삭제</button>
				</td>
			</tr>
			</c:if>
			</c:forEach>
		...
		<form method="post" action="${pageContext.request.contextPath}/admin/addComment">
			...
			<div style="text-align:right"><button class="btn btn-outline-info btn-sm" type="submit">작성</button></div>
			</div>
		</form>
		</div>
		<div class="col-6" style="text-align:right">
		<button class="btn btn-sm btn-outline-warning" onclick="location.href='${pageContext.request.contextPath}/admin/modifyNotice/${notice.noticeId}'" type="button">수정</button>
		<button class="btn btn-sm btn-outline-danger" onclick="location.href='${pageContext.request.contextPath}/admin/removeNotice/${notice.noticeId}'" type="button">삭제</button>
